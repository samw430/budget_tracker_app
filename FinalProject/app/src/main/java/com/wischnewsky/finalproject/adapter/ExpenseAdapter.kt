package com.wischnewsky.finalproject.adapter
...
import com.wischnewsky.finalproject.ScrollingActivity
...
class ExpenseAdapter : RecyclerView.Adapter<ExpenseAdapter.ViewHolder> {
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        when(viewHolder.tvCategory.text) {
            ...
            "Groceries" -> viewHolder.ivIcon.setImageDrawable(ContextCompat.getDrawable(context, R.drawable.icon))
            "Clothing" -> viewHolder.ivIcon.setImageDrawable(ContextCompat.getDrawable(context, R.drawable.icon))
            "Lodging" -> viewHolder.ivIcon.setImageDrawable(ContextCompat.getDrawable(context, R.drawable.icon))
            "Transportation" -> viewHolder.ivIcon.setImageDrawable(ContextCompat.getDrawable(context, R.drawable.icon))
            else -> viewHolder.ivIcon.setImageDrawable(ContextCompat.getDrawable(context, R.drawable.icon))

        }
        if(expense.photofile != ""){
            val imgFile = File(expense.photofile)

            if (imgFile.exists()) {

                val myBitmap = BitmapFactory.decodeFile(imgFile.getAbsolutePath())

                viewHolder.ivIcon.setImageBitmap(myBitmap)
